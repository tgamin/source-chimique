...
@section('content')
    <div class="page-content container-fluid">
        <form class="form-edit-add" role="form" action="@if($edit){{ route('voyager.'.$dataType->name.'.update', $dataTypeContent->id) }}@else{{ route('voyager.'.$dataType->name.'.store') }}@endif" method="POST" enctype="multipart/form-data">
                ...
                <div class="col-md-8">
                    ...
                    <div class="panel">
                        ...
                        <div class="panel-body">
                            @php
                                $dataTypeRows = $dataType->{($edit ? 'editRows' : 'addRows' )};
                                $exclude = ['title', 'slug', 'type', 'parent_id', 'status', 'seo_description', 'seo_title', 'order' ];
                            @endphp
                        ...
                        <div class="panel-body">
                            ...
                            <div class="form-group  @if($dataTypeContent->type!='filiale') hidden @endif" id="parentPage">
                                ...
                                <select class="select2" name="parent_id" id="parent_id">
                                    ...
                                    <option value="{{$metier->id}}" @if($dataTypeContent->parent_id==$metier->id) selected @endif > {{$metier->title}}</option>
                                    @endforeach
                                </select>
                            </div>
            ...
            @section('submit-buttons')
                <button type="submit" class="btn btn-primary pull-right">
                     @if($edit){{ __('Sauvegarder') }}@else <i class="icon wb-plus-circle"></i> {{ __('Sauvegarder') }} @endif
                </button>
            @stop
